...
describe("POST /api/videos", () => {
	test("creates video and returns 201 for valid YouTube URL", async () => {
	...
	test("returns 400 for invalid YouTube URL", async () => {
		const { handler, dispose } = createTestHandler();
	...
	test("returns 409 for duplicate video", async () => {
		const { handler, dispose } = createTestHandler({
			seedDatabase: (db) => {
		...
		try {
			...
			const data = (await response.json()) as DuplicateErrorResponse;
	...
	test("accepts various YouTube URL formats", async () => {
		const urls = [
...
describe("GET /api/videos", () => {
	test("returns empty array for user with no videos", async () => {
		const { handler, dispose } = createTestHandler();
	...
	test("returns only current user's videos", async () => {
		const { handler, dispose } = createTestHandler({
			seedDatabase: (db) => {
	...
	test("supports pagination with limit and offset", async () => {
		...
		try {
			// Test limit
			const response1 = await makeRequest(handler, {
				method: "GET",
				path: "/api/videos?limit=2",
			});

			expect(response1.status).toBe(200);
			const data1 = (await response1.json()) as VideoListResponse;
			expect(data1.videos).toHaveLength(2);
			expect(data1.limit).toBe(2);

			// Test offset
			const response2 = await makeRequest(handler, {
				method: "GET",
				path: "/api/videos?offset=3",
			});

			expect(response2.status).toBe(200);
			const data2 = (await response2.json()) as VideoListResponse;
...
describe("GET /api/videos/:id", () => {
	test("returns video details for owned video", async () => {
		const { handler, dispose } = createTestHandler({
			seedDatabase: (db) => {
				db.insert(schema.videos)
					.values({
	...
	test("returns transcript when video is completed", async () => {
		const { handler, dispose } = createTestHandler({
			seedDatabase: (db) => {
				// Add completed video
				db.insert(schema.videos)
	...
	test("returns null transcript for pending video", async () => {
		const { handler, dispose } = createTestHandler({
			seedDatabase: (db) => {
				db.insert(schema.videos)
					.values({
	...
	test("returns 403 for video owned by another user", async () => {
		const { handler, dispose } = createTestHandler({
			seedDatabase: (db) => {
...
describe("POST /api/videos/:id/retry", () => {
	test("retries failed video and returns 200", async () => {
		const { handler, dispose } = createTestHandler({
			seedDatabase: (db) => {
				db.insert(schema.videos)
					.values({
	...
	test("returns 400 when video is not in failed state", async () => {
		const { handler, dispose } = createTestHandler({
			seedDatabase: (db) => {
				db.insert(schema.videos)
					.values({
